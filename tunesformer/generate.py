...
def get_args(parser):
    ...
    parser.add_argument(
        "-max_patch", type=int, default=128, help="integer to define the maximum length in tokens of each tune"
    )
    parser.add_argument(
        "-top_p",
        type=float,
...
def generate_abc(args):
    ...
    patch_config = GPT2Config(
        num_hidden_layers=PATCH_NUM_LAYERS, max_length=PATCH_LENGTH, max_position_embeddings=PATCH_LENGTH, vocab_size=1
    )
    char_config = GPT2Config(
        num_hidden_layers=CHAR_NUM_LAYERS, max_length=PATCH_SIZE, max_position_embeddings=PATCH_SIZE, vocab_size=128
    )
    ...
    checkpoint = torch.load("weights.pth")
    fixed_weights = {k: v for k, v in checkpoint["model"].items() if not re.search("\.attn.bias|\.attn.masked_bias", k)}
    ...
    for i in range(num_tunes):
        ...
        while input_patches.shape[1] < max_patch:
            predicted_patch, seed = model.generate(
                input_patches, tokens, top_p=top_p, top_k=top_k, temperature=temperature, seed=seed
            )
            tokens = None
