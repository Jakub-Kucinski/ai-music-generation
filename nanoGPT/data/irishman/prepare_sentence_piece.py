...
VOCAB_SIZE = 1000
old_tokenizer = AutoTokenizer.from_pretrained("bert")
# old_tokenizer.all_special_tokens
...
# Prepend "<start> " and append " <end>" to each "abc notation"
for entry in train_data:
    # Strip any surrounding whitespace to be safe
    entry["abc notation"] = entry["abc notation"].strip() + " <|endoftext|>"
...
tokenizer = old_tokenizer.train_new_from_iterator([texts], VOCAB_SIZE)
...
# # Split both texts by whitespace into lists
# train_tokens = train_text.split()
# valid_tokens = valid_text.split()

# # Create a single list of all unique tokens that appear in both lists (the union)
# unique_tokens = list(set(train_tokens) | set(valid_tokens))
# vocab_size = len(unique_tokens)

# # Create mappings using enumerate: index-to-token and token-to-index
